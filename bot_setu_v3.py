...
image_config = TinyDB('./db/image_search.json')
image_table = image_config.table('image')
...
class Setu:
    ...
    def send(self):  # 判断数量
        ...
        if self.api_0_realnum == 0 and self.api_1_realnum == 0 and self.api_pixiv_realnum == 0:
            image_url = ImageGet().getImage('+'.join(self.tag))
            if image_url:
                sendMsg.send_pic(self.ctx, '伊莉丝为你找到了这个哦\n(*≧▽≦)ﾉｼ))', image_url)
            else:
...
class Getdata:
    def defaultdata(self, data):
        ...
        data['r18'] = {'group': True, 'temp': True}  # 是否开启r18
        data['freq'] = 10  # 频率 (次)
        ...
        data['revoke'] = {'group': 0, 'temp': 0}  # 撤回消息延时(0为不撤回)
        data['at'] = False  # @
...
class Command:
    ...
    def cmd_group(self, lv):  # 2<1<0   0:root
        ...
        if lv <= 2:
            ...
            elif info := re.match('_晚安~', self.ctx.Content):
                sendMsg.send_text(self.ctx, '晚安~ >_<')
                bot.close()
...
class ImageGet:
    def __init__(self):
        self.url = 'https://image.so.com/j?q='
        self.image_config = TinyDB('image_search.json')
        self.image_table = self.image_config.table('image')
    ...
    def add(self, s):
        result = self.image_table.search(Q.key_word == s)
        if result:
            self.image_table.update({'index': result[0]['index'] + 1}, Q.key_word == s)
            return result[0]['index'] + 1
        else:
            self.image_table.insert({'key_word': s, 'index': 0})
            return 0

    def getImage(self, key_word: str):
        key_word.replace(" ", "+")
        resp = requests.get(self.url + key_word, headers=self.headers(), timeout=15)
        if resp.status_code == 200:
            pic_list = json.loads(resp.content)['list']
            url = pic_list[self.add(key_word) % len(pic_list)]['img']
            return url
        return None
...
class SetuSQL:
    ...
    def __init__(self):
        pass
        # self.db = pymysql.connect(host="localhost", user="saki", password="Atm.dk09", database="setu")
    ...
    def addSetu(self, ctx):
        content = json.loads(ctx.Content)
        for pic in content['GroupPic']:
            if pic['FileSize'] > 1024 * 300:
                from_str = '来自' + ctx.FromGroupName + '@' + ctx.FromNickName
                action.send_group_pic_msg(561521251, pic['Url'], content=from_str)

                # sql = "INSERT INTO QQSetu(groupId, groupName, userId, userName, url, time) VALUES(" + "'"
                # sql += str(ctx.FromGroupId) + "','" + ctx.FromGroupName + "','" + str(ctx.FromUserId) + "','" + ctx.FromNickName
                # sql += "','" + pic['Url'] + "',now())"
                #
                # try:
                #     # 执行sql语句
...
def group_setu(ctx: GroupMsg):
    # if ctx.FromUserId == 1608835058:
    #     sendMsg.send_text(ctx, '都灵里奈爪巴', True)
    # elif ctx.FromUserId == 2270816244:
    #     sendMsg.send_text(ctx, 'QwQ', True)
    # else:
    #     sendMsg.send_text(ctx, '已经没有涩图了QAQ', False)
...
url_weiqu = 'http://gchat.qpic.cn/gchatpic_new/2270816244/561521251-2534335053-2D6802FC06B12061D7547D476E660C68/0' \
            '?vuin=3168225839 '
usr_list_pa = [2270816244, 2967349114, 1744331671, 1608835058]
...
@bot.on_group_msg
@deco.not_botself
@deco.in_content(pattern_gou)
@deco.only_this_msg_type('AtMsg')
def group_msg(ctx: GroupMsg):
    dic = json.loads(ctx.Content)
    if config['botQQ'] in dic['UserID']:
        sendMsg.send_text(ctx, '🐶 🍐 🍳 ➕ 🆙 ④🐜 ，♟🎶 🔥 🦇 👃 🏻 🎼 👆  QwQ', False)


content_wangyiyun = r'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + \
                    r'<msg templateID="123" url="http://music.163.com/song/4942751/?userid=86316998" serviceID="1" ' \
                    r'action="" actionData="" a_actionData="" i_actionData="" brief="[分享]落叶 (ゲーム「装甲悪鬼村正」EDテーマ)" ' \
                    r'flag="0">' + \
                    r'<item layout="2"><picture cover="http://p1.music.126.net/RrmXoeyq82NnicDoXhFrQg' \
                    r'==/5855998929672342.jpg"/><title>落叶 (' \
                    r'ゲーム「装甲悪鬼村正」EDテーマ)</title><summary>いとうかなこ<summary></item><source url="" icon="" name="网易云音乐" ' \
                    r'appid="100495085" action="" actionData="" a_actionData="tencent100495085://" ' \
                    r'i_actionData=""/></msg>[分享]落叶 (' \
                    r'ゲーム「装甲悪鬼村正」EDテーマ)\nいとうかなこ\nhttp://music.163.com/song/4942751/?userid=86316998\n来自: 网易云音乐 '


@bot.on_group_msg
@deco.not_botself
@deco.in_content('(.*?)网[易抑]云(.*?)')
@deco.only_this_msg_type('AtMsg')
def group_msg(ctx: GroupMsg):
    dic = json.loads(ctx.Content)
    if config['botQQ'] in dic['UserID']:
        action.send_group_pic_msg(561521251, url_weiqu)
        logger.info('wangyiyun')
...
def clock_morning():
    if datetime.now().weekday() < 5:
        action.send_group_pic_msg(214326592, picUrl='http://wx1.sinaimg.cn/large/006cSBLKly1fyny2keqnqg309q09qabt.gif',
                                  content='我是早起提醒小助手，请看到这条消息的人和我一起早起搬砖吧\nQAQ')
        action.send_group_pic_msg(214326592, picUrl=url_weiqu)
    # 如果需要循环调用，就要添加以下方法
    timer = threading.Timer(86400, clock_morning)
    timer.start()
...
if __name__ == '__main__':
    if os.path.isfile('.bot_setu_v3_flag'):  # 有文件
        ...
        timer = threading.Timer(timer_start_time, clock_morning)
        timer.start()
